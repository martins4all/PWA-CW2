...
publicVapidKey = 'BCoauDSn3nhKF0gW5VGE7yCbKyn-CrVIq51Wrj80KchaMxF4py_0s4EnD7l4nKmWQ9l8uVwjAARAEpE9s_nFkVM';

privateVapidKey = 'DtJtki0UimGjR8ibBCZKp7i6t3UhXPShiJEVA9zrLk0';
...
app.get("/", (req, res) => {
  res.sendFile(__dirname + '/sw/login.html');
...
app.get("/user_success", (req, res) => {
  res.sendFile(__dirname + '/sw/user_success.html');

}); 

//user search page
app.get("/search", (req, res) => {
  res.sendFile(__dirname + '/sw/search.html');
...
dbo1.collection('userContact').save({name: u_name, email:u_email ,password:u_pass, usertype:u_usertype}, (err, result) => {
...
app.get('/sign_up', function(req, res) {
  ...
  res.sendFile(__dirname + '/sw/sign_up.html') //create a index file 
...
res.sendFile(__dirname + '/sw/user_success.html') //create a index file 
...
MongoClient.connect('mongodb+srv://user1:password1234@cluster0-bfovl.mongodb.net/test?retryWrites=true&w=majority', function(err, db) {
  if(!err) {
  p = db
...
var query = { email: u_email };
  ...
  dbo.collection('userContact').find(query).toArray(function(err, results) {
...
dbo1.collection('userContact').save({Name: u_name, Email:u_email ,Password:u_pass}, (err, result) => {
   if (err) return console.log(err)
